...
const EditProductAdmin = ({ close ,data : propsData,fetchProductData}) => {
  ...
  const handleChange = (e) => {
    ...
    if (numericFields.includes(name)) {
      newValue = Math.max(0, Number(value)); // prevent negative
    }
    ...
    setData((prev) => {
      ...
      if (name === "discount" && Number(newValue) > Number(prev.price)) {
        updatedData.discount = prev.price;
      }
  
      if (name === "price" && Number(newValue) < Number(prev.discount)) {
        updatedData.discount = newValue; // adjust discount if price drops
      }
