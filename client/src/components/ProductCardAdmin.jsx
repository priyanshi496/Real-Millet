...
const ProductCardAdmin = ({ data, fetchProductData }) => {
  ...
  return (
    <div className="w-52 p-4 bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-200 flex flex-col justify-between m-2">
      {/* Image Box */}
      ...
      <div className="flex-grow flex flex-col justify-between text-center mt-2">
        ...
        <p className="text-slate-500 text-sm">{data?.unit} Unit</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mt-3">
        <button
          ...
          className="border px-2 py-1 text-sm border-green-600 bg-green-100 text-green-800 hover:bg-green-200 rounded-md"
        >
          Edit
        </button>
        <button
          onClick={() => setOpenDelete(true)}
          className="border px-2 py-1 text-sm border-red-600 bg-red-100 text-red-600 hover:bg-red-200 rounded-md"
        >
          Delete
        </button>
      </div>
